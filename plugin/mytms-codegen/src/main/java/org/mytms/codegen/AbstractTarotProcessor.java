...
public abstract class AbstractTarotProcessor extends AbstractProcessor {
    ...
    private EntityType handleEntityType(TypeElement element) {
        ...
        if (null != typeArguments) {
            typeArray = new Type[typeArguments.size()];
            for (int index = 0; index < typeArguments.size(); index++) {
                typeArray[index] = new SimpleType(declareToType(typeArguments.get(index)));
            }
        }
...
//        processingEnv.getMessager().printMessage(Diagnostic.Kind.ERROR, packageName);
        if (packageName.startsWith("java.lang")) {
            return new SimpleType(className);
        } else {
...
//        }
        ...
        for (Element element : getElements(Entity.class)) {
